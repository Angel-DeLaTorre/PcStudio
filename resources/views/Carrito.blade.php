...
@section('content')
                ...
                <div class="row ">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="row ml-1">
                            ...
                            echo '<p><del>$' . $producto->precioVenta . '</del></p>' . '<p class="text-danger ml-2"> $' .
                                $nuevoCosto . ' ¡Precio de oferta!</p>';
                            ?>
                            <input hidden type="text" name="txtprecioVenta"
                                id="txtprecioVenta<?php echo $j; ?>"
                                value="{{ $nuevoCosto }}" />
                            <?php
                            } else {
                            echo '<p>$' . $producto->precioVenta . '</p>'; ?>
                            <input hidden hidden type="text" name="txtprecioVenta"
                                id="txtprecioVenta<?php echo $j; ?>"
                                value="{{ $producto->precioVenta }}" />
                            <?php
                            }
                            if ($producto->cantidad <= 0) { echo '<p class="ml-4 text-danger cardItem">Agotado</p>' ; } ?> </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="row">
                                            ...
                                            <select name="cantidad"
                                                id="cantidad<?php echo $j; ?>"
                                                class="form-control col-lg-1 col-md-1 form-control-sm"
                                                onchange="ShowSelected();">
                                                @for ($i = 1; $i <= $producto->cantidad; $i++)
                                                    <option value="{{ $producto->cantidadProducto }}" @if ($producto->cantidadProducto == $i)
                                                        selected
                                                @endif>{{ $i }}</option>
            @endfor
            </select>
    ...
    <input type="text" hidden name="txtIdProducto[]" id="txtIdProducto<?php echo $j; ?>"
        value="{{ $producto->idProducto }}">

    <input type="text" hidden name="txtCantidad[]" id="txtCantidad<?php echo $j; ?>"
        value="{{ $producto->cantidadProducto }}">

    <input type="text" hidden name="txtSubtotal[]" id="txtSubtotal<?php echo $j; ?>"
        value="{{ $subtotal }}">
    ...
    <div class="col-lg-2">
        <a href="{{ route('deleteProductoCarrito', $producto->idCarrito) }}">
            <p class="btn btn-outline-danger">Quitar de carrito</p>
        </a>
        <a href="{{ route('guardarProducto', $producto->idCarrito) }}">
            <p class="btn btn-outline-primary">Guadar para mas tarde</p>
        </a>
    </div>
    </div>
    <hr>
    ...
    <h3 id="hTotal1">Total ${{ number_format($total, 2) }} MXN</h3>
    <h2 id="hTotal2"></h2>
    <hr>
            ...
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    <div class="row ml-1">
                        ...
                        echo '<p><del>$' . number_format($lp->precioVenta, 2) . '</del></p>' . '<p class="text-danger ml-2">
                            $' . number_format($nuevoCosto, 2) . ' ¡Precio de oferta!</p>';
                        } else {
                        echo '<p>$' . number_format($lp->precioVenta, 2) . '</p>';
                        }
                        if ($lp->cantidad <= 0) { echo '<p class="ml-4 text-danger">Agotado</p>' ; } ?>
                            </div>
                            <div>
                                <div class="form-group row">
                                    ...
                                    <select name="cantidad" id="cantidad"
                                        ...
                                        @for ($i = 1; $i <= $lp->cantidad; $i++)
                                            <option value="{{ $lp->cantidadProducto }}" @if ($lp->cantidadProducto == $i)
                                                selected
                                        @endif>{{ $i }}</option>
        @endfor
        </select>
    ...
    <div class="col-lg-2">
        <a href="{{ route('deleteProductoCarrito', $lp->idCarrito) }}">
            <p class="btn btn-outline-danger">Quitar de carrito</p>
        </a>
        <a href="{{ route('asignarCompra', $lp->idCarrito) }}">
            <p class="btn btn-outline-primary">Agregar a compra</p>
        </a>
    </div>
    </div>
